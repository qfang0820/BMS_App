...
with tab_bms_events:
    ...
    if bms_df is None:
        if bms_error:
            ...
            st.info("Upload a **BMS log** in the sidebar to see events.")
    else:
        df = bms_df
        pack_v_col = "Stack voltage"
        current_col = "Stack current"
        ...
        low_cell_events = df[df[min_cell_v_col] < MIN_CELL_CRIT].copy()
        low_soc_events = df[df[soc_col] < SOC_CRIT].copy()
        delta_events = df[df["cell_delta"] > DELTA_WARN].copy()

        colA, colB, colC = st.columns(3)
        colA.metric("Low Cell Events (< 2.50 V)", len(low_cell_events))
        colB.metric("Low SoC Events (< 5%)", len(low_soc_events))
        colC.metric("High Delta Events (> 0.30 V)", len(delta_events))

        st.markdown("### Low Cell Voltage Events")
        if low_cell_events.empty:
            st.info("No low cell voltage events.")
        else:
            st.dataframe(
                low_cell_events[
                    ["__time__", min_cell_v_col, max_cell_v_col, "cell_delta", soc_col, current_col]
                ].rename(
                    columns={
                        "__time__": "Time",
                        min_cell_v_col: "Min Cell (V)",
                        max_cell_v_col: "Max Cell (V)",
                        "cell_delta": "Delta (V)",
                        soc_col: "SoC (%)",
        ...
        st.markdown("### Low SoC Events")
        if low_soc_events.empty:
            st.info("No low SoC events.")
        else:
            st.dataframe(
                low_soc_events[
                    ["__time__", soc_col, min_cell_v_col, max_cell_v_col, "cell_delta", current_col]
                ].rename(
                    columns={
                        "__time__": "Time",
                        soc_col: "SoC (%)",
        ...
        st.markdown("### High Cell Delta Events")
        if delta_events.empty:
            st.info("No high cell delta events.")
        else:
            st.dataframe(
                delta_events[
                    ["__time__", "cell_delta", min_cell_v_col, max_cell_v_col, soc_col, current_col]
                ].rename(
                    columns={
                        "__time__": "Time",
                        "cell_delta": "Delta (V)",
...
with tab_bms_summary:
    ...
    if bms_df is None:
        if bms_error:
            ...
            st.info("Upload a **BMS log** in the sidebar to generate a summary.")
    else:
        df = bms_df
        pack_v_col = "Stack voltage"
        current_col = "Stack current"
        ...
        lines = []
        lines.append(f"Data time range: {fmt_time(start_time)} to {fmt_time(end_time)}.")
        lines.append(
            f"Stack voltage ranged from {df[pack_v_col].min():.1f} V to {df[pack_v_col].max():.1f} V."
        )
        lines.append(
            f"Stack current ranged from {df[current_col].min():.1f} A to {df[current_col].max():.1f} A."
        )
        lines.append(
            f"SoC ranged from {df[soc_col].min():.1f} % to {df[soc_col].max():.1f} %."
        )
        lines.append(
            f"Cell voltages ranged from {df[min_cell_v_col].min():.3f} V (min cell) "
            f"to {df[max_cell_v_col].max():.3f} V (max cell)."
        )
        lines.append(
            f"The maximum cell voltage delta (MAX - MIN) observed was {df['cell_delta'].max():.3f} V."
        )
        lines.append("")

        if low_cell_events.empty and low_soc_events.empty and delta_events.empty:
            lines.append(
                "No critical low cell voltage, low SoC, or high cell delta events were detected "
                "based on current thresholds."
            )
        else:
            if not low_cell_events.empty:
                first_low = low_cell_events.iloc[0]
                lines.append(
                    f"- Low cell voltage events detected ({len(low_cell_events)} occurrences). "
                    f"First event at {fmt_time(first_low['__time__'])} with min cell "
                    f"{first_low[min_cell_v_col]:.3f} V and delta {first_low['cell_delta']:.3f} V."
                )
            if not low_soc_events.empty:
                first_soc = low_soc_events.iloc[0]
                lines.append(
                    f"- Low SoC events detected ({len(low_soc_events)} occurrences). "
                    f"First event at {fmt_time(first_soc['__time__'])} with SoC "
                    f"{first_soc[soc_col]:.1f} % and min cell {first_soc[min_cell_v_col]:.3f} V."
                )
            if not delta_events.empty:
                first_d = delta_events.iloc[0]
                lines.append(
                    f"- High cell delta events detected ({len(delta_events)} occurrences). "
                    f"First event at {fmt_time(first_d['__time__'])} with delta "
                    f"{first_d['cell_delta']:.3f} V (min {first_d[min_cell_v_col]:.3f} V, "
                    f"max {first_d[max_cell_v_col]:.3f} V)."
                )

        lines.append("")
        lines.append(
            "This summary is auto-generated from the uploaded BMS data. "
            "Please review and adjust wording before sending to the customer."
        )

        summary_text = "\n".join(lines)

        st.text_area("Summary text (editable)", summary_text, height=260)

        st.download_button(
            label="⬇ Download Summary as TXT",
            data=summary_text,
            file_name="bms_summary.txt",
            mime="text/plain",
        )
...
with tab_cells:
    ...
    st.write(
        "You can analyze cell data in **two ways**:\n"
        "1. Upload **one combined file** (all racks in one file) using the sidebar upload.\n"
        "2. Or choose how many racks you have and upload **one file per rack** below "
        "(each file like `Time, Serial number, V1..V396`)."
    )

    mode = st.radio(
        "Choose cell data input mode:",
        ["Single file with all racks (sidebar upload)", "Separate file per rack"],
        key="cell_mode",
    )

    # ---------- MODE 1: Single combined cell file ----------
    if mode == "Single file with all racks (sidebar upload)":
        if cell_file is None and cell_df_raw is None:
            st.info("Upload a **cell-level file** in the sidebar to use this mode.")
        elif cell_error is not None:
            st.error(cell_error)
            if cell_df_raw is not None:
                st.dataframe(cell_df_raw.head(50))
        elif cell_df_raw is not None:
            df_cells = cell_df_raw.copy()

            st.markdown("### Raw Cell Data Preview")
            st.dataframe(df_cells.head(50))
            st.caption(f"Rows: {df_cells.shape[0]}, Columns: {df_cells.shape[1]}")
            ...
            with col_map1:
                rack_col = st.selectbox(
                    "Rack column (rack ID / name)",
                    options=df_cells.columns,
                    key="single_rack_col",
                )
                cell_id_col = st.selectbox(
                    "Cell ID / Position column",
                    options=df_cells.columns,
                    key="single_cell_id_col",
                )

            with col_map2:
                cell_v_col = st.selectbox(
                    "Cell Voltage column",
                    options=df_cells.columns,
                    key="single_cell_v_col",
                )
                temp_col = st.selectbox(
                    "Temperature column (optional)",
                    options=["(none)"] + list(df_cells.columns),
                    key="single_temp_col",
                )

            st.markdown("### ⚙ Value Scaling for Cell Voltage")
            scale_mV_single = st.checkbox(
                "Cell voltage is stored in mV (e.g., 3480 = 3.480 V) → divide by 1000",
                value=True,
                key="single_scale_mV",
            )

            df_cells["__cell_v__"] = pd.to_numeric(df_cells[cell_v_col], errors="coerce")

            if scale_mV_single:
                df_cells["__cell_v__"] = df_cells["__cell_v__"] / 1000.0

            if temp_col != "(none)":
                df_cells["__temp__"] = pd.to_numeric(df_cells[temp_col], errors="coerce")
            else:
                df_cells["__temp__"] = None

            df_cells = df_cells.dropna(subset=["__cell_v__"])
            ...
            if df_cells.empty:
                ...
                rack_stats = (
                    df_cells.groupby(rack_col)["__cell_v__"]
                    .agg(["count", "min", "max", "mean"])
                    .reset_index()
                )
                rack_stats["delta"] = rack_stats["max"] - rack_stats["min"]

                rack_stats = rack_stats.rename(
                    columns={
                        rack_col: "Rack",
                        "count": "Cell Count",
                        "min": "Min Cell V",
                ...
                st.dataframe(rack_stats)
                st.caption(
                    "Delta = Max cell voltage - Min cell voltage within each rack. "
                    "Larger delta means more imbalance."
                )

    # ---------- MODE 2: Separate file per rack (Time + Serial number + V1..V396) ----------
    else:
